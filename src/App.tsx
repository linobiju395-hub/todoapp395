...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    ...
    // Ensure any stale mock items with dummy titles or IDs are purged
    ...
    const cleanTodos = loaded.filter(
  ...
  const handleRemoveDuplicates = async () => {
    ...
    addToast(`Removed ${deleteIds.length} duplicate task${deleteIds.length > 1 ? 's' : ''}`, 'success');
  };
