    ...
    <aside className="w-full md:w-56 shrink-0 flex flex-col gap-6 py-1 select-none">
      ...
      <div className="space-y-0.5">
        ...
        {viewItems.map((item) => {
          ...
          // Only active when viewing by views (not a specific category)
          const active = currentCategory === 'all' && currentView === item.id;
      ...
      <div className="space-y-0.5">
        ...
        <button
          ...
          className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-md text-xs transition-colors cursor-pointer ${
            currentCategory === 'all' && currentView === 'all'
              ? 'bg-neutral-200/70 dark:bg-white/[0.08] text-neutral-900 dark:text-white font-medium'
              : 'text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-white/[0.04]'
          }`}
        >
          <span>All categories</span>
          <span className="text-[11px] font-mono text-neutral-400 dark:text-neutral-500">{allCount}</span>
        </button>
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between px-2 mb-1">
          ...
          {currentPriority !== 'all' && (
            <button
              ...
              className="text-[10px] text-neutral-500 hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white transition-colors cursor-pointer underline"
            >
              Clear
            </button>
          )}
        ...
        <div className="flex flex-wrap gap-1 px-2">
          {(['all', 'urgent', 'high', 'basic', 'low'] as const).map((p) => {
            const active = currentPriority === p || (p === 'basic' && (currentPriority as string) === 'medium');
            return (
              <button
                key={p}
      ...
      {/* Subtle Progress Metric & Clean Slate option */}
      {allCount > 0 && (
        <div className="mt-auto px-2 pt-4 border-t border-neutral-200/80 dark:border-white/[0.06] text-xs text-neutral-500 space-y-2">
          ...
          {onClearAllTasks && (
            <button
              id="sidebar-clear-all-tasks-btn"
              type="button"
              onClick={onClearAllTasks}
              className="pt-2 text-[11px] text-neutral-400 hover:text-rose-500 dark:hover:text-rose-400 flex items-center gap-1.5 transition-colors cursor-pointer"
            >
